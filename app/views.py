...
class ProductDetailView(View):
	def get(self, request, pk):
		totalitem = 0
		# getting product through pk(i.e id)
...
def searchMatch(query, item):
    if query in item.title.lower() or query in item.category.lower() or query in item.description.lower() or query in item.brand.lower():
        return True
    else:
        return False
...
def search(request):
    ...
    if len(allprods)==0 or len(query)<2:
        return render(request, 'app/search.html', {'msg':'Enter a valid search query','totalitem':totalitem})
    return render(request, 'app/search.html', {'allprods':allprods,'totalitem':totalitem})
...
def orders(request):
	...
	print(request.user)
	return render(request, 'app/orders.html', {'order_placed':op,'user':request.user})
...
def topwear(request, data=None):
	...
	if data==None :
			topwears = Product.objects.filter(category='TW')
	elif data == 'Bajaj' or data == 'Safira':
			topwears = Product.objects.filter(category='TW').filter(brand=data)
	elif data == 'below':
			topwears = Product.objects.filter(category='TW').filter(discounted_price__lt=1500)
	elif data == 'above':
			topwears = Product.objects.filter(category='TW').filter(discounted_price__gt=1500)
...
class ProfileView(View):
	...
	def post(self, request):
		...
		if form.is_valid():
			...
			reg = Customer(user=usr, name=name, phoneNumber=phoneNumber,locality=locality, city=city, state=state, zipcode=zipcode)
			reg.save()
